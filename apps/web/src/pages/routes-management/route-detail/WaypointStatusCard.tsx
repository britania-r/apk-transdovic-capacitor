// File: apps/web/src/pages/routes-management/route-detail/WaypointStatusCard.tsx
import type { WaypointDetail } from './useRouteDetail';
import type { LiveCollection } from './hooks/useLiveCollections';
...
export const WaypointStatusCard = ({ waypoint, collection, onClick }: Props) => {
  const status = collection?.status || 'pending';
  const config = STATUS_CONFIG[status] || STATUS_CONFIG.pending;
  const farm = waypoint.farm;

  return (
    <button className={`${styles.statusCard} ${styles[config.className] || ''}`} onClick={onClick}>
      ...
      <div className={styles.statusCardContent}>
        ...
        <div className={styles.statusCardMeta}>
          ...
          {!collection && (
            <span className={styles.statusMetaItem}>
              <i className="bx bx-droplet"></i> {(waypoint.planned_pickup_amount || 0).toLocaleString()} L plan.
            </span>
          )}
        </div>
      </div>

      <div className={styles.statusCardBadge}>
        <span className={`${styles.statusLabel} ${styles[config.className] || ''}`}>
          {config.label}
        </span>
      </div>

      <i className="bx bx-chevron-right" style={{ color: 'var(--color-text-muted)', fontSize: '1.1rem' }}></i>
    </button>
  );
};